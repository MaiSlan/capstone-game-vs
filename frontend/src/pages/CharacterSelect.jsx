...
// --- THE STANDARD 8-DIRECTION FRAMES ---
...
// --- THE FIX: CUSTOM PALADIN FRAMES ---
// The Paladin's export is missing South-East, so we duplicate East for index 1
...
// Map the correct frame arrays to each character
const ROSTER_UI = [
  ...
  return (
    <div
      ...
      style={{
        ...
        width: '256px',                
        height: '256px',
        transform: 'scale(0.85)', // THE FIX: Increased from 0.55 to make sprites much larger
...
export default function CharacterSelect({ selectedCharacter, setSelectedCharacter }) {
  ...
  return (
    <div className="bg-pure-abyss min-h-screen pt-32 pb-12 font-grim relative text-zinc-400">
      ...
      <main className="w-full max-w-6xl mx-auto px-6 z-10 flex flex-col items-center relative">
        
        {/* --- THE FIX: Updated to the Branded Descent Theme --- */}
        ...
        <div className="flex flex-wrap justify-center gap-8 mb-14">
          {ROSTER_UI.map((char) => (
            <div 
              ...
              // THE FIX: Made cards significantly taller and wider (w-52 h-72)
              className={`w-52 h-72 flex flex-col items-center justify-between p-4 cursor-pointer transition-all duration-300 relative
                bg-black/60 backdrop-blur-md
                ${selectedCharacter === char.id 
                  ? 'border border-red-700 shadow-[0_0_30px_rgba(139,0,0,0.6)] scale-105 transform translate-y-[-8px]' 
            ...
            >
              ...
              <div className="flex-1 w-full flex items-center justify-center overflow-visible">
                {char.hasRotation ? (
                  <InteractiveSprite
                    ...
                    frames={char.frames} // Passes custom frames if applicable
              ...
              <div className="w-full text-center py-3 mt-4 border-t border-red-900/30 font-royal font-bold text-sm uppercase tracking-widest text-zinc-200">
        ...
        <div className="h-48 w-full max-w-3xl flex flex-col items-center justify-center mb-12">
          {activeCharUI && activeCharUI.status === 'unlocked' && activeCharData ? (
            <div className="w-full flex flex-col items-center animate-fade-in">
              {/* --- THE FIX: Brighter, highly legible quote text --- */}
              <p className="text-sm md:text-base text-zinc-100 drop-shadow-[0_2px_8px_rgba(220,38,38,0.8)] tracking-[0.15em] font-semibold mb-8 text-center italic bg-black/40 px-8 py-3 rounded-md border-l-2 border-r-2 border-red-900/50">
                "{activeCharData.quotes[quoteIndex]}"
              </p>
              
              <div className="flex w-full justify-center gap-16 border-t border-b border-red-900/30 py-5 bg-black/20">
                <div className="flex flex-col items-center">
                  <span className="text-xs uppercase tracking-[0.4em] text-zinc-500 mb-2 font-bold">Vitality</span>
                  <span className="font-royal text-2xl text-red-100 drop-shadow-[0_0_5px_rgba(255,0,0,0.5)]">{activeCharData.hp}</span>
                </div>
                <div className="w-px bg-red-900/40 flex items-center justify-center relative">
                  <span className="absolute text-red-900/60 text-[10px]">✦</span>
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-xs uppercase tracking-[0.4em] text-zinc-500 mb-2 font-bold">Agility</span>
                ...
                <div className="flex flex-col items-center">
                  <span className="text-xs uppercase tracking-[0.4em] text-zinc-500 mb-2 font-bold">Relic</span>
                  <span className="font-royal text-xl text-red-100 drop-shadow-[0_0_5px_rgba(255,0,0,0.5)]">{activeCharData.weaponName}</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="text-xs uppercase tracking-[0.5em] text-zinc-700 font-bold">Awaiting Vessel...</div>
          )}
        </div>

        <div className="flex gap-12 items-center mt-auto pb-8">
          <button 
          ...
          <button 
            ...
            // --- THE FIX: Updated button terminology to "Brand Them" ---
