    ...
    <nav className="nav-gothic w-full h-16 flex items-center justify-between px-8 md:px-12 fixed top-0 z-50 font-grim">
      ...
      <div className="hidden md:flex items-center gap-12 text-[10px] uppercase tracking-[0.3em] font-bold z-10">
        {!isAuthenticated ? (
          // Links for visitors
          <>
            <a href="#dogma" className="text-zinc-500 hover:text-red-800 transition-colors duration-300">Dogma</a>
            <a href="#architecture" className="text-zinc-500 hover:text-red-800 transition-colors duration-300">Architecture</a>
            <a href="#relics" className="text-zinc-500 hover:text-red-800 transition-colors duration-300">Relics</a>
          </>
        ) : (
          // Links for logged-in players (instead of just disappearing!)
          <>
            <span onClick={() => navigate('/leaderboard')} className="text-zinc-500 hover:text-zinc-200 cursor-pointer transition-colors duration-300">Leaderboard</span>
            <span onClick={() => navigate('/compendium')} className="text-zinc-500 hover:text-zinc-200 cursor-pointer transition-colors duration-300">Compendium</span>
          </>
        )}
      </div>
        ...
        {!isAuthenticated ? (
          location.pathname !== '/auth' && (
            <button 
              ...
              className="text-[10px] font-bold tracking-[0.3em] uppercase text-zinc-400 hover:text-red-600 transition-colors duration-300"
            >
              Sign In / Register
            </button>
          )
